  ...
  return next => action => {
    ...
    switch (action.type) {
      ...
      case types.CHANGE_CURRENT_DIR: {
        ...
        // This ensures that refresh state won't get canceled before update is
        // finished.
        ...
        for (let idx = 0; idx < expression.length; idx = idx + 1) {
          ...
          if (expr.tag === "title") {
            result = result.filter(
              r => r.title.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          } else if (expr.tag === "artist") {
            result = result.filter(
              r => r.artist.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          } else if (expr.tag === "album") {
            result = result.filter(
              r => r.album.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          } else if (expr.tag === "albumartist") {
            result = result.filter(
              r => r.albumArtist.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          } else if (expr.tag === "date") {
            result = result.filter(
              r => r.year.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          } else if (expr.tag === "genre") {
            result = result.filter(
              r => r.genre.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          } else if (expr.tag === "filename") {
            result = result.filter(
              r => r.name.toLowerCase().indexOf(
                expr.value.toLowerCase()
              ) !== -1
            )
          }
