...
const listToChildren = (list) => {
  return list.filter(
    (el) => ('directory' in el || 'file' in el || 'playlist' in el)
  ).map((element) => {
    let node = {}
    ...
    switch (action.type) {
      case types.CONNECT: {
        ...
        client.mpd.connect(action.host, action.port).then(() => {
          ...
          client.disconnects.push(
            client.mpd.onPlayerUpdate(handlePlayerUpdate(store))
          )

          // Subscribe to queue events.
          client.disconnects.push(
            client.mpd.onQueueUpdate(handleQueueUpdate(store))
          )

          // Subscribe to queue events.
          client.disconnects.push(
            client.mpd.onPlaylistUpdate(handlePlaylistUpdate(store))
          )

          // Subscribe to error events.
          client.disconnects.push(
            client.mpd.onError(handleErrorUpdate(store))
          )

          // Subscribe to close events.
          client.disconnects.push(
            client.mpd.onDisconnected(handleClose(store))
          )
      ...
      case types.STATUS_UPDATED: {
        ...
        if (
          ...
          client.updatingProgress &&
          client.mpd.connected
        ) {
          client.progressTimeout = setTimeout(
        ...
        if ('status' in action &&
          'songid' in action.status &&
          action.status.songid != store.getState().status.songid
        ) {
      ...
      case types.CHANGE_CURRENT_DIR: {
        ...
        if (!action.force && node !== null && node.children.length > 0) {
          ...
          const path = action.path.length < 2
            ? '/'
            : action.path.slice(1).reduce((result, item) => result + '/' + item)
      ...
      case types.ADD_TO_QUEUE: {
        ...
        nodesToPaths(items).then((allFiles) => {
          ...
          for (let index = 0; index < filePaths.length; index++) {
            filesWithPositions.push(
              { file: filePaths[index], position: position + index}
            )
          }
      ...
      case types.LOAD_SONGS: {
        ...
        const expression = [
          ...
          // I believe it's better user experience to return full
          // album list.
          { tag: 'album', value: album },
        ]
